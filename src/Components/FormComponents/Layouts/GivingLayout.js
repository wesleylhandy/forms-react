...
class GivingLayout extends Component {
    ...
    renderArray(amounts, selectedIndex, type) {
        return amounts.map((amount, i)=>(
            <AskArrayBtn key={`array${i}`} className={`askbutton${type == "tabs" ? "__tabs" : ""} ${selectedIndex == i ? "selected" : ""}`} onClick={()=>this.addToCart(amount, i)}>
                <div className={`askbutton__amt${type == "tabs" ? "--tabs" : ""}`}>${amount}</div>
            </AskArrayBtn>
        ))
    }
        ...
        return givingFormat === "buttons" ? (
                <FieldSet>
                    <legend>Giving Amounts and Giving Options</legend>
                        <FormHeader className="askarray__header">Select A {monthlyChecked ? "Monthly" : "Single"} Donation Amount</FormHeader>
                        ...
                        <OtherGiftAmountGroup id="OtherGiftAmount" className="askarray--other">
                            <div id="OtherAmount" className={`askarray__form-group--other ${selectedIndex == 99 ? "styles.selected": ""}`}>
                                <label className="form-group__other-input--label" htmlFor="other-amt-input">Other Amount</label>
                                <input 
                                    key={key} 
                ...
                <FieldSet>
                    ...
                    <OtherGiftAmountGroup id="OtherGiftAmount" className="askarray__tabs--other">
                        <div id="OtherAmount" className={`askarray__form-group--tabs ${selectedIndex == 99 ? " styles.selected": ""}`}>
                            <label className="form-group-tabs__other-input--label" htmlFor="other-amt-input">Or specify amount</label>
